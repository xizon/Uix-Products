		...
		?>
			<tr>
				<th class="uix-cmb__title">
					...
					<?php if ( !empty ( $desc ) ) { ?>
					    <p class="uix-cmb__title_desc"><?php echo UixProducts::kses( $desc ); ?></p>
					<?php } ?>
				</th>
				<td>
				...
				<td>
					   ...
					   <textarea placeholder="<?php echo esc_attr( $placeholder ); ?>" rows="<?php echo absint( $rows ); ?>" cols="40" name="<?php echo esc_attr( $id ); ?>" id="<?php echo esc_attr( $id ); ?>"><?php echo esc_textarea( $default ); ?></textarea>
						<?php if ( !empty ( $desc_primary ) ) { ?>
							<p class="uix-cmb__description"><?php echo UixProducts::kses( $desc_primary ); ?></p>
						<?php } ?>
			...
			<tr>
				...
				<td>
					   ...
					   <input data-format="<?php echo esc_attr( $format ); ?>" placeholder="<?php echo esc_attr( $placeholder ); ?>" type="text" class="uix-cmb__short-text uix-cmb__date-selector" value="<?php echo esc_attr( $default ); ?>" name="<?php echo esc_attr( $id ); ?>" id="<?php echo esc_attr( $id ); ?>">
						<?php if ( !empty ( $desc_primary ) ) { ?>
							<span class="uix-cmb__description"><?php echo UixProducts::kses( $desc_primary ); ?></span>
						<?php } ?>
						...
						if ( is_array ( $options )  && isset( $options[ 'value' ] ) ) {
							...
							foreach ( $options[ 'value' ] as $key => $value  ) {
								...
								<?php if ( $radio_type == 'normal' ) { ?>
									<label data-value="<?php echo esc_attr( $key ); ?>" data-toggle-id="<?php echo esc_attr( $toggle_id ); ?>" class="<?php if ( $br ) { echo 'uix-cmb__label'; } else { echo ''; }; ?> uix-cmb__radio-text uix-cmb__toggle-selector <?php if ( $default == esc_attr( $key ) || empty( $default ) ) { echo 'active'; } else { echo ''; }; ?>"><input type="radio" name="<?php echo esc_attr( $id ); ?>_r" value="<?php echo esc_attr( $key ); ?>" <?php echo esc_html( $checked ); ?>/><?php echo UixProducts::kses( $value ); ?></label>
								<?php } ?>
