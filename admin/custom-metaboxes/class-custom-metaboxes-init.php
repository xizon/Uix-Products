 ...
 * Custom Metaboxes
 *
 * @class 		: Uix_Products_Uix_Custom_Metaboxes
...
if ( !class_exists( 'Uix_Products_Uix_Custom_Metaboxes' ) ) {
	...
	class Uix_Products_Uix_Custom_Metaboxes {
		...
		*/
		...
		public static function wp_core() {
			 ...
			 if ( 
				 UixProducts::inc_str( $currentScreen->id, 'uix_products' ) || 
				 UixProducts::inc_str( $currentScreen->id, 'uix-products' ) || 
			 ...
			 ) 
			 {
				    ...
				    wp_enqueue_script( 'wp-color-picker-alpha', UixProducts::plug_directory() .'admin/js/wp-color-picker-alpha.min.js', array( 'wp-color-picker' ), '2.1.2', true );
		...
		?>
			<tr>
				<th class="custom_metabox_title">
					...
					<?php if ( !empty ( $desc ) ) { ?>
					    <p class="custom_metabox_title_desc"><?php echo UixProducts::kses( $desc ); ?></p>
					<?php } ?>
				</th>
				<td>
				...
				<td>
					   ...
					   <textarea placeholder="<?php echo esc_attr( $placeholder ); ?>" rows="<?php echo absint( $rows ); ?>" cols="40" name="<?php echo esc_attr( $id ); ?>" id="<?php echo esc_attr( $id ); ?>"><?php echo esc_textarea( $default ); ?></textarea>
						<?php if ( !empty ( $desc_primary ) ) { ?>
							<p class="custom_metabox_description"><?php echo UixProducts::kses( $desc_primary ); ?></p>
						<?php } ?>
			...
			<tr>
				...
				<td>
					   ...
					   <input data-format="<?php echo esc_attr( $format ); ?>" placeholder="<?php echo esc_attr( $placeholder ); ?>" type="text" class="custom_short_text custom_date_selector" value="<?php echo esc_attr( $default ); ?>" name="<?php echo esc_attr( $id ); ?>" id="<?php echo esc_attr( $id ); ?>">
						<?php if ( !empty ( $desc_primary ) ) { ?>
							<span class="custom_metabox_description"><?php echo UixProducts::kses( $desc_primary ); ?></span>
						<?php } ?>
						...
						if ( is_array ( $options )  && isset( $options[ 'value' ] ) ) {
							...
							foreach ( $options[ 'value' ] as $key => $value  ) {
								...
								<?php if ( $radio_type == 'normal' ) { ?>
									<label data-value="<?php echo esc_attr( $key ); ?>" data-toggle-id="<?php echo esc_attr( $toggle_id ); ?>" class="<?php echo ( $br ) ? 'custom_label_block' : ''; ?> custom_radio_text custom_toggle_selector <?php echo ( $default == esc_attr( $key ) || empty( $default ) ) ? esc_attr( 'active' ) : ''; ?>"><input type="radio" name="<?php echo esc_attr( $id ); ?>_r" value="<?php echo esc_attr( $key ); ?>" <?php echo esc_html( $checked ); ?>/><?php echo UixProducts::kses( $value ); ?></label>
								<?php } ?>
		...
		* @print: 
			...
			if ( is_array( $_data ) && sizeof( $_data ) > 0 ) {
				...
				?>
					<li>
						<strong><?php echo UixProducts::kses( $value[ 'name' ] ); ?></strong>
						<span>
							<?php echo UixProducts::kses( $value[ 'value' ] ); ?>
						</span>
					</li>
				<?php
				}
