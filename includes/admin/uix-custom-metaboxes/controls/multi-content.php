		...
		if ( is_array( $_data ) && sizeof( $_data ) > 0 ) {
			...
			foreach( $_data as $index => $value ) {
				...
				if ( is_array( $value ) && sizeof( $value ) > 0 ) {
					...
					//Parent Category
	                $parent = Uix_Products_Custom_Metaboxes::parse_jsondata_from_editor( $value[ 'parent' ] );
	
					?>
					<section class="slide <?php echo ( !empty($parent) ? 'slide-child' : ''); ?> <?php echo esc_attr( Uix_Products_Custom_Metaboxes::parse_jsondata_from_editor( $value[ 'classname' ] ) ); ?>" id="<?php echo esc_attr( Uix_Products_Custom_Metaboxes::parse_jsondata_from_editor( $value[ 'id' ] ) ); ?>" data-parent="<?php echo esc_attr( $parent ); ?>">

						<h3><?php echo esc_html( Uix_Products_Custom_Metaboxes::parse_jsondata_from_editor( $value[ 'name' ] ) ); ?></h3>
						<?php echo Uix_Products_Custom_Metaboxes::kses( Uix_Products_Custom_Metaboxes::autospace_to_front_from_jsondata( Uix_Products_Custom_Metaboxes::parse_jsondata_from_editor( $value[ 'desc' ] ) ) ); ?>
						<hr>
						<?php echo Uix_Products_Custom_Metaboxes::kses( Uix_Products_Custom_Metaboxes::parse_jsondata_from_editor( $value[ 'value' ] ) ); ?>
...
*
*/
class UixProductsCmbFormType_MultiContent extends Uix_Products_Custom_Metaboxes {
	...
	public static function add( $id = '', $title = '', $desc = '', $default = '', $options = '', $placeholder = '', $desc_primary = '', $enable_table = false ) {
		...
		$parent_res = '<select class="uix-products-cmb__text--fullwidth uix-products-cmb__text--div--toggle__sel">';
		$parent_res.= '<option value="">'.esc_html__( '-', 'uix-products' ).'</option>';
		...
		$temp = '
			...
			<div class="uix-products-cmb__text--div uix-products-cmb__text--div--toggle uix-products-cmb__text--div--toggle-customlevel">
			   ...
			   <a href="javascript:void(0);" class="uix-products-cmb__custom-attributes-field__removebtn" title="'.esc_attr( $label_upbtn_remove ).'"><svg aria-hidden="true" role="img" focusable="false" class="dashicon dashicons-trash" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20"><path d="M12 4h3c.6 0 1 .4 1 1v1H3V5c0-.6.5-1 1-1h3c.2-1.1 1.3-2 2.5-2s2.3.9 2.5 2zM8 4h3c-.2-.6-.9-1-1.5-1S8.2 3.4 8 4zM4 7h11l-.9 10.1c0 .5-.5.9-1 .9H5.9c-.5 0-.9-.4-1-.9L4 7z"></path></svg></a>
				...
				<div class="uix-products-cmb__text--div--toggle__div">
					...
					<label class="uix-products-cmb__text--row">
						...
						<input type="text" class="uix-products-cmb__text--fullwidth uix-products-cmb__text--div--toggle__title" name="'.esc_attr( $id ).'_attrs_title[]" value="{name}"><span class="important2">*</span>
					</label>
					...
					<label class="uix-products-cmb__text--row" style="display:none;">
						...
						<input type="text" class="uix-products-cmb__text--fullwidth uix-products-cmb__text--div--toggle__id" name="'.esc_attr( $id ).'_attrs_id[]" value="{id}">
					</label>
					...
					<label class="uix-products-cmb__text--row">
						...
						<input type="hidden" class="uix-products-cmb__text--div--toggle__toggleSelect" name="'.esc_attr( $id ).'_attrs_parent[]" value="{parent}">
						<div class="uix-products-cmb__text--div--toggle__toggleSelect__append">'.$parent_res.'</div>
					</label>    
					...
					<label class="uix-products-cmb__text--row">
						...
						<input type="text" class="uix-products-cmb__text--fullwidth uix-products-cmb__text--div--toggle__classname" name="'.esc_attr( $id ).'_attrs_classname[]" value="{classname}">
					</label>   
					...
					<label class="uix-products-cmb__text--row">
						...
						<textarea class="uix-products-cmb__text--div--toggle__textarea" rows="3" name="'.esc_attr( $id ).'_attrs_desc[]" >{desc}</textarea>  
	...
	?>
		...
		<?php endif; ?>	
				...
				<!-- Begin Fields -->
				<div class="uix-products-cmb__wrapper uix-products-cmb__custom-attributes-field uix-products-cmb__custom-attributes-field--customlevel <?php echo esc_attr( $one_column? 'uix-products-cmb__wrapper--one-column' : '' ); ?>" data-append-id="<?php echo esc_attr( $id ); ?>_append" data-tmpl='<?php echo esc_attr( $temp_attr ); ?>'>
					...
					<?php endif; ?>	
								...
								<textarea style="display: none;" class="uix-products-cmb__custom-attributes-field__resultReverseData" name="<?php echo esc_attr( $id ); ?>_resultReverseData" ><?php echo esc_textarea( $all_reverse_data_res ); ?></textarea>
								<textarea style="display: none;" class="uix-products-cmb__custom-attributes-field__resultData" name="<?php echo esc_attr( $id ); ?>_resultData" ><?php echo esc_textarea( $all_data_res ); ?></textarea>
