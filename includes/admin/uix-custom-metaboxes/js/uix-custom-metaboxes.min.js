...
$this.data('click',0);if(_closebtn){jQuery(_closebtn).off('click');jQuery(document).on('click',_closebtn,function(event){event.preventDefault();jQuery('#'+tid).val('');jQuery('#'+pid).find('img').attr('src','');jQuery('#'+pid).hide();jQuery(this).hide();$this.show();$this.data('click',1);});}});});return this;},customAttrs:function(selector,max){jQuery(document).ready(function(){jQuery(selector).each(function(){var $this=jQuery(this),$addButton=$this.find('.uix-products-cmb__custom-attributes-field__addbtn'),wrapperID='#'+$this.data('append-id'),x=1,maxField=max,fieldHTML=$this.data('tmpl');$addButton.off('click.UixProductsCustomMetaboxes_customAttrs_add').on('click.UixProductsCustomMetaboxes_customAttrs_add',function(e){e.preventDefault();if(x<maxField){x++;var uuid=UixProductsGUID.create();jQuery(wrapperID).append(fieldHTML.replace(/\{id\}/g,uuid));jQuery.when(jQuery(wrapperID).length>0).then(function(){UixProductsCustomMetaboxesInit.toggleSelect();UixProductsCustomMetaboxesInit.editor();});}
